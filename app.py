...
LANGS = {
    "en": {
        ...
        "intro": "Merge multiple PDF files from a single .rar archive with a clean, simple workflow.",
        ...
        "rar_error": (
            ...
            "It may be damaged, password-protected, or unsupported."
        ),
        "pdf_error": (
            "No valid PDF could be merged. "
            "Please check that the files are not damaged or protected."
        ),
        "step1_title": "Upload",
        ...
        "private_text": "Files are processed temporarily and are not stored permanently.",
    ...
    "es": {
        ...
        "intro": "Une múltiples archivos PDF desde un único archivo .rar con un flujo limpio y simple.",
    ...
    "pt": {
        ...
        "intro": "Mescle vários arquivos PDF a partir de um único arquivo .rar com um fluxo limpo e simples.",
        ...
        "rar_error": (
            "Não foi possível extrair o arquivo .rar. "
            "Ele pode estar danificado, protegido por senha ou em formato não suportado."
        ),
        "pdf_error": (
            "Nenhum PDF válido pôde ser mesclado. "
            "Verifique se os arquivos não estão danificados ou protegidos."
        ),
        "step1_title": "Enviar",
...
def normalize_language(value: Optional[str]) -> str:
    """Convierte es-CO, es-ES, pt-BR, pt-PT o en-US en es, pt o en."""
    if not value:
        return "en"
...
def get_query_lang() -> Optional[str]:
    """Lee ?lang=es, ?lang=pt o ?lang=en desde la URL."""
    try:
        raw = st.query_params.get("lang", None)
...
def get_browser_locale() -> str:
    """Obtiene el idioma del navegador cuando Streamlit lo permite."""
    try:
...
def img_to_base64(path: str) -> str:
    """Convierte una imagen local en base64 para insertarla en HTML."""
...
if logo_main_b64:
    mini_logo_html = f'<img src="data:image/png;base64,{logo_main_b64}" alt="No-Limit_PDF">'
else:
...
if logo_main_b64:
    st.markdown(
        ...
        <div class="hero">
            ...
            <div class="hero-subtitle">{T["intro"]}</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
if logo_company_b64:
    company_logo_html = f'<img src="data:image/png;base64,{logo_company_b64}" alt="RelaxLife-Apps">'
else:
